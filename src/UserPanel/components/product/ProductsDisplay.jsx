...
export default function ProductsDisplay() {
  ...
  // Update URL whenever filters or sorting change
  useEffect(() => {
    const params = {};
    for (const [key, values] of Object.entries(filtersState)) {
      if (values.length > 0) {
...
const handleFilterUpdate = (value, category) => {
  ...
  const handleRadioFilterChange = (value, category) => {
    setFiltersState((prevState) => ({ ...prevState, [category]: [value] }));
  };
  ...
  const filteredKurtis = applyFilters(women_kurtis);
  const sortedKurtis = applySorting(women_kurtis);
        ...
        {section.options.map((option, idx) => (
          <div key={option.value} className="flex items-center">
            <input
              ...
              name={`${section.id}`}
              value={option.value}
              checked={filtersState[section.id]?.includes(option.value) || false}
              onChange={() => handleFilterUpdate(option.value, section.id)}
              className="size-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
            />
        ...
        {section.options.map((option, idx) => (
          <div key={option.value} className="flex items-center">
            <input
              ...
              name={`${section.id}`}
              value={option.value}
              checked={filtersState[section.id]?.[0] === option.value}
